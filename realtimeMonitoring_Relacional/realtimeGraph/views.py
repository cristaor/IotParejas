...
def get_measurements_by_stations_json(request, **kwargs):
    data_result = {}

    measureParam = kwargs.get("measure", None)
    selectedMeasure = None
    #modelo tabla medidas
    measurements = Measurement.objects.all()
    ...
    for station in stations:
        #stationData = Station.objects.filter(station=station)

        for measurement in measurements:
            print(str(station.id) + " " + measurement.name)
            stationData = Data.objects.filter(
                station__id=station.id, measurement__name=measurement.name,  time__gte=start.date(), time__lte=end.date())
            if stationData.count() <= 0:
                continue

            data.append({
                'Station': f'{station.id}',
                'Measurement': f'{measurement.name}',
                'DataCount': stationData.count()
            })

    startFormatted = start.strftime("%d/%m/%Y") if start != None else " "
    endFormatted = end.strftime("%d/%m/%Y") if end != None else " "

    data_result["stations"] = [stat.str() for stat in stations]
    data_result["start"] = startFormatted
    data_result["end"] = endFormatted
    data_result["data"] = data
