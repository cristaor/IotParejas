...
// Definiciones
/*Constantes de configuración del display Oled de 7 pines*/
...
//Igresamos parametros de sensor de Luminosidad
int dht_dpin2 = 0;        //Pin de lectura de datos AO
const long A = 1000;     //Resistencia en oscuridad en KΩ
const int B = 15;        //Resistencia a la luz (10 Lux) en KΩ

// WiFi
// Nombre de la red WiFi
const char ssid[] = "HOME-DA19"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "0F0FE28673E28C4"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "54.160.139.200"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
const int MQTT_PORT = 8082;
const char MQTT_USER[] = "suscriptor1";
//Contraseña de MQTT
const char MQTT_PASS[] = "987654.."; // TODO Reemplace ContrasenaMQTT por la contraseña correpondiente al usuario especificado.

//Tópico al que se recibirán los datos
// El tópico de publicación debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/out
const char MQTT_TOPIC_PUB[] = "colombia/cundinamarca/bogota/suscriptor1/out"; //TODO Reemplace el valor por el tópico de publicación que le corresponde.
// El tópico de suscripción debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/in
const char MQTT_TOPIC_SUB[] = "colombia/cundinamarca/bogota/suscriptor1/in"; //TODO Reemplace el valor por el tópico de suscripción que le corresponde.
...
//Cambio Valor de medicion de la luminosidad
float lumi;
...
void sendSensorData(float temperatura, float humedad, float luminosidad) {
  ...
  data += "\"luminosidad\": "+ String(luminosidad, 1);   //Cambio
  data += "}";
...
/*Cambio  para obtener luminosidad*/
float readLuminosidad()
{
  int analogValue = analogRead(LUXPIN);
  int resistanceValue = 10; //we are using a 10K ohms resistor
...
bool checkMeasures(float t, float h, float l) {
  // Se comprueba si ha habido algún error en la lectura
    if (isnan(t) || isnan(h) || isnan(l)) {
      Serial.println("Error obteniendo los datos del sensor DHT21 o el fotoresistor");
      return false;
    }
    return true;
}
void TextDisplay()
{
  ...
  display.println("Reto Capa Logica");
  display.display();
  delay(3000);
}

void DrawTriangle(String mensaje)
{
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  ...
  display.fillRoundRect(25, 25, 10, 20, 5, SSD1306_WHITE);
  display.fillCircle(30, 50, 5, SSD1306_WHITE);
  display.display();
  delay(2000);  
}
